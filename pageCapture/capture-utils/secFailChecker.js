...
const checkFailData = async (driver, obj, isMobile, badgeData) =>{
    ...
    const buttons = await driver.executeScript(function () {
        const result = [];
        const btns = document.querySelectorAll('.swiper-wrapper button');
        btns.forEach((button) => {
    ...
    // 5. 텍스트 오류의 경우
        ...
        // if(str == "CO05"){
            ...
            // co05 이미지
            if (selectedElement) {
                const matchingElements = await driver.executeScript((textType, tileNumber, isMobile, isLLL,selectedElement, desc,isBadgeBig) => {
